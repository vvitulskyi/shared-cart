...
class SharedCart {
    constructor() {
        ...
        this.router.get("/shared-cart/connection/:link", checkAuth, async (req, res) => {
            try {
                ...
                if (user.shared_carts.map((o) => o.toString()).includes(cart.cart_id)) {
                    const userObject = user.toObject();
                    const { password_hash, _id, ...userDoc } = userObject;
        ...
        this.router.post("/shared-cart/link/create", checkAuth, async (req, res) => {
            try {
        ...
        this.router.get("/shared-cart/:cart_id/norm", checkAuth, async (req, res) => {
            try {
                const { user_id, params } = req;
                const { cart_id } = params;
                const c_id = new ObjectId(cart_id);
        ...
        this.router.patch("/shared-cart/add-item", checkAuth, async (req, res) => {
            try {
                ...
                await cart.save();
                const countedProducts = this.quantityFromCart(products, cart.items);
                req.wss && req.wss.clients.forEach((client) => {
        ...
        // Edit item quantity
        this.router.patch("/shared-cart/set-quantity", checkAuth, async (req, res) => {
            try {
        ...
        this.router.patch("/shared-cart/clear", checkAuth, async (req, res) => {
            try {
                ...
                cart.items = new Types.DocumentArray([]);
                await cart.save();
                req.wss && req.wss.clients.forEach((client) => {
                    if (client.readyState === WebSocket.OPEN) {
    ...
    editQantity(items, product_id, quantity, isAdding = false) {
        const productIndex = items.findIndex(({ item_id }) => item_id == product_id);
        if (!quantity) {
...
//# sourceMappingURL=SharedCart.js.map